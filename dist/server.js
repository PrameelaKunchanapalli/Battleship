...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function isValidPlacement(newShip, ships) {
        return newShip.every((pos) => pos >= 0 && pos < gridSize * gridSize && !ships.has(pos) // Ensure position is within bounds and not occupied
        );
    }
    ...
    function placeShipsRandomly(ships, grid, isPlayer = false) {
        ...
        shipShapes.forEach((shape) => {
            ...
            while (!placed) {
                ...
                const newShip = shape.map((offset) => isVertical ? startPosition + offset * gridSize : startPosition + offset);
    ...
    function handlePlayerAttack(cell) {
        ...
        if (cell.classList.contains("hit") || cell.classList.contains("miss"))
            return; // Ignore if already attacked
    ...
    function handleComputerTurn() {
        if (availableMoves.size === 0)
            return; // Stop if no moves are left
        let index; // Declare as 'undefined' to handle uninitialized case
